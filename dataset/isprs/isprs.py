...
class IsprsTrainDataSet(data.Dataset):
    """
       IsprsTrainDataSet is employed to load train set
       Args:
        root: the isprs dataset path,
         isprs
          ├── gtFine
          ├── leftImg8bit
        list_path: isprs_train_list.txt, include partial path
    ...
    def __init__(self, root='', list_path='', max_iters=None, base_size=513, crop_size=513, mean=(128, 128, 128),
        ...
        for name in self.img_ids:
            ...
            name = name.strip().split()[0].strip().split('/', 3)[2].split('.')[0]
...
class IsprsValDataSet(data.Dataset):
    """
       isprsDataSet is employed to load val set
       Args:
        root: the isprs dataset path,
         isprs
          ├── gtFine
          ├── leftImg8bit
        list_path: isprs_val_list.txt, include partial path
...
class IsprsTestDataSet(data.Dataset):
    ...
    def __init__(self, root='', list_path='', crop_size=513, mean=(128, 128, 128), std=(128, 128, 128),
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, name.split()[0])
            label_file = osp.join(self.root, name.split()[1])
            name = name.strip().split()[0].strip().split('/', 2)[1].split('.')[0]
            self.files.append({"img": img_file, "label": label_file, "name": name})
