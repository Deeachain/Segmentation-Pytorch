...
class EESPNet_Seg(nn.Module):
    def __init__(self, num_classes=19, s=2, pretrained=None, gpus=1):
        super().__init__()
        classificationNet = EESPNet(num_classes=1000, s=s)
        if gpus >=1:
        ...
        self.project_l1 = nn.Sequential(nn.Dropout2d(p=p), C(self.net.level1.act.num_parameters + num_classes, num_classes, 1, 1))
...
if __name__ == '__main__':
    ...
    model = EESPNet_Seg(num_classes=19, s=2).to(device)
