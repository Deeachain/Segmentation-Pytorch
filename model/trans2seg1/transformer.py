...
class Attention_Decoder(nn.Module):
    ...
    def forward(self, q, x):
        ...
        q = self.fc_q(q).reshape(B, self.num_heads, n_class, D // self.num_heads) # [B, num_head, class, 256/num_head]
        kv = self.fc_kv(x).reshape(B, HW, 2, self.num_heads, D // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1] # [B, num_head, HW, D/num_head]

        attn1 = (q @ k.transpose(-2, -1)) * self.scale #[B, num_head, class, HW]
        attn2 = attn1.softmax(dim=-1)
        attn3 = self.attn_drop(attn2) #[B, num_head, class, HW]

        query = (attn3 @ v).reshape(B, n_class, D)  #[B, num_head, class, D/num_head] -> [B, class, D]
        # query = self.proj(query)
        query = self.proj_drop(query)  # [B, class, D]

        # attn = attn3.permute(0, 2, 1, 3)
        attn = attn1.permute(0, 2, 1, 3) # [B,num_class,head,HW]
        # attn = attn2.permute(0, 2, 1, 3)
        return query, attn  # [B,num_class,D]  [B,num_class,head,HW]
...
class Block_Decoder(nn.Module):
    ...
    def __init__(self, dim, num_heads, feat_HxW, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.norm4 = norm_layer(feat_HxW)
    ...
    def forward(self, query, feat):
        # query:[B,class,D] feat:[B,HW,D]
        query, attn = self.attn(self.norm1_clsembed(query), self.norm1(feat))
        ...
        # q*k, q*k'*v, encoder_output
        return attn, query, feat # [B, num_head, class, HW]; [B,num_class,D]; [B,HW,D]
        # [B, class, num_head, HW]; [B,num_class,D]; [B,HW,D]
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, input_dim=2048, embed_dim=512, depth=12, num_patches=16*16, num_classes=6,
                 feat_HxW=1024, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        self.cls_embed = nn.Parameter(torch.zeros(1, num_classes, embed_dim))
        ...
        self.blocks_decoder = nn.ModuleList([
            Block_Decoder(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, feat_HxW=feat_HxW, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)])
