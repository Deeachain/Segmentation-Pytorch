...
"""
Time:     2021/1/12 上午9:42
Author:   Ding Cheng(Deeachain)
File:     NFSNet_nogfr.py
...
class NFSNet_nogrf(nn.Module):
    def __init__(self, num_classes, backbone=resnet18):
        super(NFSNet_nogrf, self).__init__()
    ...
    def forward(self, x):
        ...
        sat1 = self.sat1(layers[-1])
        sat2 = self.sat2(layers[-2])

        output1 = self.conv_2(sat1)
        ...
        output = torch.add(torch.add(output1, sat2), layers_last2)

        # output = torch.add(output1, sat2)
...
if __name__ == '__main__':
    model = NFSNet_nogrf(num_classes=3, backbone=resnet18)
